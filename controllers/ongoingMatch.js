...
const roundDetails = async (roundId) => {
  ...
  /*
  concept for scoreCard while playingMatch
  we have to send all 18holes or 9holes data

  */
  const scoreCard = await ongoingMatch.find({
    //_id: { $lte: mongoose.Types.ObjectId(roundId) }

  }).select('players isRoundCompleted holeNo');
  ...
  //implemeting scoreCard
  resData.scoreCard=scoreCard;
...
const recordScore = async (req, res) => {
  ...
  //update isRoundCompleted field from false to true
  //because round has been finally played/completed
  await ongoingMatch.findByIdAndUpdate(roundId,{
    $set:{
